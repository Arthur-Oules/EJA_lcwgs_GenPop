...
```{r}
pops_coast_isj <- c("bodega_bay",    "big_creek",         "elkhorn",
                    "pacific_grove", "isla_san_jeronimo", "laguna_beach",
                    "point_dume",    "santa_barbara",     "la_jolla_san_diego",
                    "tomales_bay",   "redondo_beach",     "santa_cruz_harbour")
)
...
p3 <- mantel_plot(as.dist(geosphere::distm(coordinates_coast_isj)), Edward_distance_coast_isj, xlab = "Euclidean", ylab = "Chord distance")
...
```{r}
pops_coast <- c("bodega_bay",    "big_creek",          "elkhorn",
                "pacific_grove", "laguna_beach",       "point_dume",
                "santa_barbara", "la_jolla_san_diego", "tomales_bay",
                "redondo_beach", "santa_cruz_harbour")
...
cross_entropies <- tibble(
  `c-e`   = map(4:9, \(x) cross.entropy(EJA_snmf_analysis, K = x) |> as.vector()) |> unlist(),
  K_value = factor(c(rep(4, 50), rep(5, 50), rep(6, 50), rep(7, 50), rep(8, 50), rep(9, 10)))
)
