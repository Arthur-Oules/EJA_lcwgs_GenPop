...
# Outliers with `genomics_general`
...
EJA_vcf <- read.vcfR(here("data", "8-Embiotoca_filtered.vcf.gz"))

geno <- extract.gt(EJA_vcf) # Character matrix containing the genotypes
position <- getPOS(EJA_vcf) # Positions in bp
chromosome <- getCHROM(EJA_vcf) # Chromosome information
...
pop_num <- c(
  ...
  rep(4,   6), # ELK
  rep(5,  10), # GUA
  rep(6,  13), # HOP -> Pacific Grove PGR 
  rep(7,  11), # ISJ
  rep(8,  13), # LB
  rep(9,   5), # PD
  rep(10, 13), # SB
...
EJA_outFLANK <- list("position" = position, "chromosome" = chromosome, "G" = G, "pop" = pop)

rm(geno, position, chromosome, pop, G)
saveRDS(EJA_outFLANK, file = here("data", "8-Embiotoca_filtered_outflank.rds"))
```

```{r}
EJA_outFLANK <- readRDS(here("data", "8-Embiotoca_filtered_outflank.rds"))

EJA_Fst <- MakeDiploidFSTMat(
  t(EJA_outFLANK$G),
  locusNames = EJA_outFLANK$position,
  popNames   = EJA_outFLANK$pop
)
head(EJA_Fst)

saveRDS(EJA_Fst, here("data", "outFLANK_Fst.rds"))
```

```{r}
EJA_Fst <- readRDS(here("data", "outFLANK_Fst.rds"))
```

```{r}
EJA_Fst |> ggplot(aes(x = He, y = FST)) + 
  geom_bin2d(bins = 100) +
...
plot(EJA_Fst$He, EJA_Fst$FST, alpha = .1)
```

```{r}
EJA_Fst |> ggplot(aes(x = FST, y = FSTNoCorr)) + 
  geom_bin2d(bins = 100) +
  scale_fill_viridis(option = "turbo") +
  geom_abline(slope = 1, color = "red") +
  theme_classic()

plot(EJA_Fst$FST, EJA_Fst$FSTNoCorr)
abline(0, 1)
```

```{r}
trimmed <- OutFLANK(EJA_Fst, NumberOfSamples = 15, qthreshold = 0.05, Hmin = .1)

OutFLANKResultsPlotter(trimmed, withOutliers = TRUE,
...
```{r}
hist(trimmed$results$pvaluesRightTail)
```
```{r}
P1 <- pOutlierFinderChiSqNoCorr(
  EJA_Fst,
  Fstbar     = trimmed$FSTNoCorrbar,
  dfInferred = trimmed$dfInferred,
  qthreshold = 0.05,
  Hmin       = 0.1
)
...
library(bigstatsr)
```
Filter neutral SNPs to infer neutral ditribution

```{r}
chrom_to_int <- read_tsv(here("data", "JAKOON01_catalog", "JAKOON01_contigs.tsv")) |> 
  arrange(desc(length)) |> 
  select(sequence_name, accession) |> 
...
```{r}
temp <- t(EJA_outFLANK$G)
temp[temp == 9 ] <- NA

keep <- which(colSums(is.na(temp)) == 0)

rm(temp)

G <- add_code256(
  big_copy(t(EJA_outFLANK$G[keep, ]), type = "raw"),
  code = bigsnpr:::CODE_012
)

newpc <- snp_autoSVD(
  G = G,
  infos.chr = left_join(tibble(accession = EJA_outFLANK$chromosome[keep]), chrom_to_int) |> pull(num),
  infos.pos = EJA_outFLANK$position[keep]
)
