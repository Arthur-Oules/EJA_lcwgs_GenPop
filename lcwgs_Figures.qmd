...
chromosome_map <- here_d_lcwgs("JAKOON01_catalog", "sequence_report.tsv") |> 
  ...
  filter(`GenBank seq accession` %in% (SNP_positions_pvalues
                                  |> distinct(CHROM)
                                  |> pull(CHROM))) |> 
  add_row(
...
manhattan_plot_lcwgs <- manhattan_plot_custom_2(
  SNP_positions_pvalues,
  pvalues = pvalues_all
)
```
...
manhattan_plot_lcwgs_coast <- manhattan_plot_custom_2(
  SNP_positions_pvalues,
  pvalues = pvalues_coast
)
```
...
manhattan_lcwgs_coast_descending <- SNP_positions_pvalues |>
  order_descencing() |> 
...
ggsave(
  here("output", "plots", "lcwgs",
       "manhattan_plot_custom_lcwgs_arranged_coast.png"),
...
rm(manhattan_lcwgs_coast_descending)
```
...
ggsave(
  ...
  plot   = manhattan_lcwgs_islands_mapped,
  width  = 6000,
  height = 2000,
  units  = "px"
)

rm(manhattan_lcwgs_islands_mapped)
```

#### With outliers couloured

```{r}
outliers_pos <- readRDS(
  file = here_d_lcwgs("SNP_positions.rds")
) |> 
  select(-c(lab_position, coloured, rank)) |> 
  mutate(
    CHROM      = factor(CHROM, levels = chromosome_length$CHROM),
...
manhattan_lcwgs_coast_descending_outliers <- SNP_positions_desc_order |> 
  manhattan_plot_custom_2(
    pvalues  = SNP_positions_desc_order$pvalues_coast,
    outliers = outliers_pos
  )

ggsave(
  here("output", "plots", "lcwgs",
       "manhattan_plot_custom_lcwgs_arranged_coast_outliers.png"),
  plot   = manhattan_lcwgs_coast_descending_outliers,
  width  = 6000,
  height = 2000,
  units  = "px"
)
...
manhattan_plot_lcwgs_SD_SC <- SNP_positions |>
...
chromosome_length <- SNP_positions |>
  count(CHROM) |>
  arrange(desc(n))

SNP_positions_desc_order <- SNP_positions_pvalues |>
...
rm(SNP_positions_pvalues)

manhattan_lcwgs_SD_SC_descending <- manhattan_plot_custom_2(
  SNP_positions_desc_order,
  SNP_positions_desc_order$pvalues_SD_SC
)
```
...
sample_sites$Sampling_sites[6] <- "Big Sur"
