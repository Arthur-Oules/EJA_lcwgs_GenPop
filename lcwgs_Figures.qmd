...
PCA_crop <- PCA + xlim(-.065, -.0325) + ylim(.095, .1275) + # old crop = xlim(-.065, -.05) + ylim(.1125, .1275) +
  coord_fixed(ratio = 1) +
  theme(
...
sample_sites <- read_csv2(here("data","lcwgs", "low_coverage_sampling.csv")) |>
  ...
  arrange(Popmap) |> 
  select(Longitude, Latitude, Sampling_sites, lab_lon, lab_lat) |> 
  mutate(
    Sampling_sites = gsub("\\n", "\n", Sampling_sites, fixed = TRUE) |>
      factor(ordered = TRUE)
  )

popmap_lcwgs$long_names
