...
SNP_positions <- readRDS(file = here_d_lcwgs("SNP_positions.rds"))

chromosome_map <- here_d_lcwgs("JAKOON01_catalog", "sequence_report.tsv") |> 
  read_tsv() |> 
...
admixture_data_6 <- read_table(
  here_d_lcwgs("8-Embiotoca_filtered.snmf", "K6", "run16",
...
admixture_data_7 <- read_table(
  here_d_lcwgs("8-Embiotoca_filtered.snmf", "K7", "run46",
...
geno2lfmm(
  ...
  here_d_lcwgs("8-Embiotoca_filtered.lfmm")
)
```
```{r}
indiv <- here_d_lcwgs("Individual_list.rds") |> read_rds() |> unique()

coords <- read_csv2(here_d_lcwgs("low_coverage_sampling.csv")) |> 
...
coordinates |> write_csv2(file = here_d_lcwgs("individuals_coordinates.csv"))
rm(indiv, coords, coordinates)
```

```{r}
genotype <- here_d_lcwgs("8-Embiotoca_filtered.geno") |>
  read.geno()

genotype[genotype == 9] <- NA

coordinates <- here_d_lcwgs("individuals_coordinates.csv") |>
  read_csv2() |>
...
ne_states(
  ...
  subst(from = 1, to = NA) |> 
  subst(from = 2, to = 1) |> 
  writeRaster(here_d_lcwgs("ocean_map.tif"))
```
...
gg_obj <- ggtess3Q(
  ...
  background      = TRUE,
  raster.filename = here_d_lcwgs("ocean_map.tif"),
