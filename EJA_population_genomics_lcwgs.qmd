...
## Generating whitelist for Island *vs.* Coast
...
save_open_plot(
  here("output", "plots", "coast_PCA_2_3.pdf"),
  plot   = coast_PCA_2_3,
  width  = 10,
  height = 8
)
...
rm(EJA_lcwgs_coast, coast_PCA_1_2, coast_PCA_2_3, coast_manhattan)
```

## Coastal populations -ISJ
### Generating bed file
```{bash}
cd C:/Users/aoules/Desktop/R_projects/Surfperch_GenPop/plink2

./plink2 --bfile 8-Embiotoca_filtered --keep whitelist_coastal-isj.txt --make-bed --out 8-Embiotoca_filtered_coastal-isj
```
...
EJA_lcwgs_coast_isj <- read.pcadapt(
  here("data", "SNPs", "8-Embiotoca_filtered_coastal-isj.bed"),
  type = "bed"
)
...
pcadapt_lcwgs_coast_isj <- pcadapt(EJA_lcwgs_coast_isj, K = 2)
```

```{r}
#| eval: false

write_rds(pcadapt_lcwgs_coast_isj, file = here("output", "pcadapt_lcwgs_coast-isj.rds"))
```
...
save_open_plot(
  here("output", "plots", "coast-isj_PCA_1_2.pdf"),
  plot   = coast_isj_PCA_1_2,
  width  = 10,
  height = 8
)

plot(pcadapt_lcwgs_coast_isj, option = "qqplot") + theme_minimal()
...
save_open_plot(
  here("output", "plots", "coast_manhattan-isj.png"),
...
rm(EJA_lcwgs_coast_isj, coast_isj_PCA_1_2, coast_isj_manhattan)
```
...
EJA_lcwgs_islands <- here("data", "SNPs", "8-Embiotoca_filtered_blacklisted.bed") |> 
...
save_open_plot(
  here("output", "plots", "islands_PCA_1_2.pdf"),
...
save_open_plot(
  here("output", "plots", "islands_manhattan.pdf"),
...
```{r}
EJA_lcwgs_coast_vcf <- read.vcfR(here("plink2", "8-Embiotoca_filtered_coastal.vcf"))
...
```{r}
EJA_lcwgs_coast_vcf_diff <- vector(mode='list', length=3)
parameters <- c(.5, .75, 1)

for (i in 1:3) {
  EJA_lcwgs_coast_vcf_diff[[i]] <- alleleFreqDiff(EJA_lcwgs_coast_vcf,
                                                  pm = popmap_lcwgs_coast,
                                                  p1 = "BBA", p2 = "ISJ",
                                                  difference = parameters[i])
}

rm(EJA_lcwgs_coast_vcf)
gc()
```
...
for (i in 1:3) {
  hi_het_coast[[i]] <- hybridIndex(EJA_lcwgs_coast_vcf_diff[[i]],
                                   pm = popmap_lcwgs_coast,
                                   p1 = "BBA", p2 = "ISJ")
}
```
...
for (i in 1:3) {
  hi_het_coast_plots[[i]] <- hi_het_coast[[i]] |>
    mutate(
      pop = factor(
        pop,
        levels = c("BBA", "TB", "SCH", "ELK", "HOP", "BIGC", "SB", "PD", "RB", "LAG", "SD", "ISJ")
      )
    ) |> 
    arrange(pop) |>
...
```{r}
rm(EJA_lcwgs_coast_vcf_diff, hi_het_coast, hi_het_coast_plots)
gc()
```
