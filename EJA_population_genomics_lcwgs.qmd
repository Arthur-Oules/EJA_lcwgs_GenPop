...
EJA_annotations_short <- EJA_annotations |> select(seqid, type, start, end)
...
```{r}
test <- here("output", "pcadapt_lcwgs_gua_isj.rds") |>
  readRDS() |>
  Get_outliers(method = "bonferroni", pvalue_threshold = .05) |> 
  Get_annotations(chrom_info = chromosomes_info, gff_annotation = EJA_annotations)
|>
...
cross_entropies |> filter(K_value == 7) |> mutate(run = row_number()) |> ggplot() + geom_point(aes(x = run, y = `c-e`))
```
