...
chromosome_metadata <- EJA_lcwgs_vcf |>
  getFIX() |>
  as_tibble() |>
  select(CHROM, POS) |>
  mutate(
...
chromosome_metadata |> saveRDS(file = here_d_lcwgs("manhattan_metadata.rds"))
...
rm(chromosomes_length,
   ...
   pvalues_positions_filtered_dup, pvalues_positions_filtered_dup_sequences)
gc()
```
...
EJA_lcwgs_vcf <- here_d_lcwgs("8-Embiotoca_filtered.vcf.gz") |>
  read.vcfR()

EJA_lcwgs_vcf |> write_rds(here_d_lcwgs("EJA_lcwgs_vcfR.RDS"))
