...
EJA_lcwgs <- read.pcadapt(
  here("data", "8-Embiotoca_filtered.bed"),
  type = "bed"
)
...
EJA_lcwgs_coast <- read.pcadapt(
  here("data", "8-Embiotoca_filtered_whitelisted.bed"),
  type = "bed"
)
...
EJA_lcwgs_islands <- read.pcadapt(
  here("data", "8-Embiotoca_filtered_blacklisted.bed"),
  type = "bed"
)
...
EJA_lcwgs_vcf <- here("data", "8-Embiotoca_filtered.vcf.gz") |> read.vcfR()

EJA_lcwgs_vcf |> write_rds(here("data", "EJA_lcwgs_vcfR.RDS"))

ped2geno(
  input.file  = here("plink2", "8-Embiotoca_filtered.ped"),
  output.file = here("data", "8-Embiotoca_filtered.geno")
)
