...
c("here", "tidyverse",
  ...
  "ggnewscale", "scales", "terra", "tidyterra", "tess3r") |>
...
q_matrix <- read_table(
  here("output", "admixture", "8-Embiotoca_filtered_r46.7.Q"),
...
#### Geographic ranges interpolation
```{r}
gg_obj <- ggtess3Q(
  q_matrix,
  coordinates,
...
pop_area <- gg_obj +
  ...
  scale_fill_hypso_c(
    ...
    guide   = guide_colourbar(title.vjust = 1.5)
  ) +
  geom_sf(data = states) +
  coord_sf(xlim = c(-124, -113), ylim = c(28.5, 38.5), expand = FALSE) +
  geom_point(
...
```{r}
rm(coordinates, q_matrix, catalina_df, states, sample_sites,
...
for (i in 1:3) {
  hi_het_coast_isj_plots[[i]] <- hi_het_coast_isj[[i]] |>
    ...
    labs(title = paste0("δ = ", as.character(parameters[i])))
}
```
...
EJA_snmf_analysis <- load.snmfProject(here("data", "snmf", "8-Embiotoca_filtered.snmfProject"))

snmf_tb <- tibble(
  "4" = cross.entropy(EJA_snmf_analysis, K = 4) |> as.vector(),
  "5" = cross.entropy(EJA_snmf_analysis, K = 5) |> as.vector(),
  "6" = cross.entropy(EJA_snmf_analysis, K = 6) |> as.vector(),
  "7" = cross.entropy(EJA_snmf_analysis, K = 7) |> as.vector(),
  "8" = cross.entropy(EJA_snmf_analysis, K = 8) |> as.vector(),
  "9" = cross.entropy(EJA_snmf_analysis, K = 9) |> as.vector(),
  )

snmf_tb <- snmf_tb |>
  pivot_longer(everything(), names_to = "K", values_to = "Cross-entropy") |>
  mutate(K = as.numeric(K))

snmf_plot <- snmf_tb |>
  ggplot() +
  geom_violin(aes(x = factor(K), y = `Cross-entropy`), position = "dodge", scale = "width") +
  geom_jitter(aes(x = factor(K), y = `Cross-entropy`), alpha = .1) +
  labs(x = "K") +
  theme_classic() +
  theme(
    text      = element_text(size = 20),
...
```{r}
popmap_tb <- read_csv2(here("data", "individuals_coordinates.csv"), locale = locale(encoding = "latin1")) |>
  select(
    ID     = Indiv,
    Code   = Localisation,
    pop    = popmap_clean
...
```{r}
NJ_Tree_lcwgs <- Fst_all |> as.dist() |> nj()

NJ_Tree_lcwgs$tip.label <- NJ_Tree_lcwgs$tip.label |>
  gsub("_", " ", x = _) |>
  str_to_title()
...
NJ_Tree_lcwgs_rooted <- NJ_Tree_lcwgs |> root(outgroup = "Isla Guadalupe")
