...
`8-Embiotoca_filtered.fam` contains individuals IDs in second column

```{r}
read_tsv(here("plink2", "8-Embiotoca_filtered.fam"), col_names = FALSE) |> 
  pull(X2) |> 
  write_rds(file = here("data", "Individual_list.rds"))
```

`8-Embiotoca_filtered.bim` contains SNPs chromosome in column 1 and chromosome position in column 4.

```{r}
read_tsv(here("plink2", "8-Embiotoca_filtered.bim"), col_names = FALSE) |> 
  mutate(rank = row_number()) |> 
  select(rank, X1, X4) |>
  rename("CHROM" = "X1", "POS" = "X4") |> 
  write_rds(file = here("data", "SNP_positions.rds"))
```
