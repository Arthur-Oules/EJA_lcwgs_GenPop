...
#### Clean memory
```{r}
rm(EJA_lcwgs_vcf, chromosome_names, manhattan_plot_lcwgs)
```
...
currents_tibble_annual_mean_lres <- currents_tibble_annual_mean |>
  mutate(Couples = row_number() + (row_number() %% 2)) |>
  group_by(Couples) |>
  summarise(
    x = first(x),
    y = first(y),
