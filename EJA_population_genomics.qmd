...
Get_outliers <- function(vcf_path, pcadapt, filename){
  ...
  vcf_to_catalog_ranks <- vcf |> getID() |> strsplit(split = ":") |> reduce_all_numeric()
  ...
  print(paste0("There are ", dim(outliers_phi_st)[1], " outliers with an associated phi_st."))
  outliers_phi_st_filtered <- outliers_phi_st[outliers_phi_st$phi_st > .1, ]
  print(paste0("There are ", dim(outliers_phi_st_filtered)[1], " outliers with a phi_st > 0.1."))
  ...
  sink()
  print(paste0("Outliers list saved as ", here("output", paste0(filename, "_outliers_catalog_ranks.txt"))))
...
for (i in 1:length(PCA_average$pop)){
  PCA_average$average_x[i] <- mean(PCA_df[PCA_df$pop == PCA_average$pop[i], "x"])
  PCA_average$average_y[i] <- mean(PCA_df[PCA_df$pop == PCA_average$pop[i], "y"])
}
...
PCA_average$average_y <- PCA_average$average_y - c(.04, .01, -.01, .06, .06, .06, .055, .045)

PCA_percentages <- pcadapt_all_loci$singular.values^2

# Generate plot
PCA <- ggplot() +
  geom_point(data = PCA_df, mapping = aes(x = y, y = x, fill = pop), size = 3, shape = 21) +
  geom_text(data = PCA_average, mapping = aes(x = average_y, y = average_x, label = pop)) +
  labs(
