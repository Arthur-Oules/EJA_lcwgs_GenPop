...
EJA_outliers_query_tib <- here("data", "8R1WY2TK01N-Alignment.xml") |> # "Single-file XML2" file from NCBI BLASTn output is expected
  read_xml() |>
  XML_to_df()
EJA_outliers_query_tib |> saveRDS(
...
```{r}
query_df_80_PREDICTED_unique_unique <- EJA_outliers_query_tib |>
...
EJA_outliers_protein_sequences_matches <- query_df_80_PREDICTED_unique_unique |>
...
```{r}
topo <- ggplot() +
  geom_raster(data = california_df, mapping = aes(x = x, y = y, fill = z)) +
  scale_fill_etopo() +
  # Crop map
  coord_sf(xlim = c(-124, -113), ylim = c(28.5, 38.5), expand = FALSE) +
  # Axis labels and theme
...
save_open_plot(
  here("output", "plots", "californioa_topo.pdf"),
  plot   = topo,
  width  = 9,
  height = 7
)
