...
plot(pcadapt_all_loci, option = "scores", pop = poplist_names, i = 1, j = 2)
plot(pcadapt_all_loci, option = "scores", pop = poplist_names, i = 2, j = 3)
plot(pcadapt_all_loci, option = "scores", pop = poplist_names, i = 1, j = 3)
...
EJA_outliers_ranks |> dim() |> _[1] |> print() # Plot number of outliers

outliers_sequences <- catalog_sequences |>
  filter(row_number() %in% EJA_outliers_ranks$catalog_ranks)

head(outliers_sequences)
write.csv(outliers_sequences, file = here("output", "outliers_sequences.csv"))
```

#### Save results as .fa fasta file for NCBI BLAST
...
```{r}
query_df <- here("data", "8R1WY2TK01N-Alignment.xml") |> # "Single-file XML2" file from NCBI BLASTn output is expected
  read_xml() |>
  XML_to_DF()
...
query_df_80_PREDICTED_unique_unique |>
  write.csv(file = here("output", "query_df_80_PREDICTED_unique_unique.csv"))
```
...
prot_sequences <- Get_protein_sequences(query_df_80_PREDICTED_unique_unique)

prot_sequences_matches <- prot_sequences |>
  filter(!sequences == "")
```
...
for (i in 1:dim(prot_sequences_matches[1])){
  cat(paste0(">", prot_sequences_matches$accession_number[i], "\n"), append = TRUE)
  cat(paste0(prot_sequences_matches$sequences[i], "\n"), append = TRUE)
}
sink()
```
...
```{r}
pcadapt_blacklist_raw <- read.pcadapt( # vcf file from pcadapt analysis output is expected
  here(
    "data",
    "ustacks_populations_output",
    "EJA_blacklist",
...
plot(pcadapt_whitelist, option = "scores", pop = poplist_names, i = 1, j = 2)
```
...
```{r}
manhattan_plot_custom <- function(vcf_file,
                                  pcadapt_analysis,
                                  n_chromosome,
                                  outliers_ranks = NULL) {
  chromosome_names <- vcf_file |> # Extract chromosome number from vcf info
  getFIX() |>
...
PCA_average <- data.frame("pop" = levels(as.factor(poplist_names)))
...
PCA_percentages <- pcadapt_all_loci$singular.values^2*100
```
...
PCA <- ggplot() +
  ...
  labs(
    x = paste0("-PC2: ", as.character(round(PCA_percentages[2], 2)), "%"),
    y = paste0("-PC1: ", as.character(round(PCA_percentages[1], 2)), "%")
  ) +
...
## Figure 3? - Protein sequences annotation
