...
# Functions
```{r}
reduce_all_numeric <- function(list) {
  for (i in 1:length(list)) {
    list[i] <- as.numeric(list[[i]][1])
  }
  return(list)
}

Get_outliers <- function(vcf_path, pcadapt, filename){
  vcf <- read.vcfR(paste0(vcf_path, "/populations.snps.vcf"))

  vcf_to_catalog_ranks <- vcf |>
    getID() |>
    strsplit(split = ":") |>
    reduce_all_numeric()
  ...
  outliers_phi_st <- phi_st[phi_st$catalog_rank %in% outliers_catalog_ranks, ]
  print(
    paste0(
      "There are ",
      dim(outliers_phi_st)[1],
      " outliers with an associated phi_st."
    )
  )
  outliers_phi_st_filtered <- outliers_phi_st[outliers_phi_st$phi_st > .1, ]
  print(
    paste0(
      "There are ",
...
##Read the catalog and save outliers sequences as fasta file
...
catalog_sequences <- subset(
  catalog_fa,
  rownames(catalog.fa) %in% seq(1, dim(catalog_fa)[1], 2)
)
...
## Read and extract relevant info from the BLAST results
...
for (query in xml_find_all(hit.query, ".//BlastOutput2")){ #Reads XML BLAST results as data frame
  ...
  for (Hit in xml_find_all(query, ".//Hit")){ #For each locus gets all hits
    ...
    query_df  <- rbind(
      query_df,
      c(query_id, num, accession, title, organism, bit_score, evalue)
    )
  }
}
counter <- 0
...
```{r}
prot_sequences = data.frame(# Initialisation
...
```{r}
Table_2 <-
  query_df_80_PREDICTED_unique_unique[, c("query_number", "accession_number", "title")]
...
chromosome.names <- getFIX(EJA_all_loci_vcf)[,"CHROM"] |>#Get chromosome for each vcf locus
...
ggsave(
  ...
  "open",
  args = c("-a Preview.app", here("output", "graphs", "manhattan_plot.pdf")),
  wait = FALSE
)
...
crop_temp   <- temp_matrix[(3600-1300):(3600-1100), (200+750):(400+750)] |>
  as.vector()
crop_u      <- u_matrix[(3600 - 1300):(3600 - 1100), (200 + 750):(400 + 750)] |>
  as.vector()
crop_v      <- v_matrix[(3600 - 1300):(3600 - 1100), (200 + 750):(400 + 750)] |>
  as.vector()
...
map <- ggplot() +
  ...
  geom_tile(map_data_lowres_df, aes(x = lon, y = lat, fill = temp)) +
  ...
  geom_label(
    sample_sites,
    mapping = aes(x = lab_lon, y = lab_lat, label = Sampling.location),
    hjust   = 0
  ) +
...
system2(
  "open",
  args = c("-a Preview.app", here("output", "graphs", "temp_current_map.pdf")),
  wait = FALSE
)
```

### PCA plot and map figure
...
system2(
  "open",
  args = c("-a Preview.app", here("output", "graphs", "pca_map.pdf")),
  wait = FALSE
)
```
