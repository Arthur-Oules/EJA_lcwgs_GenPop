...
```{r}
query_df <- read_xml(here("data", "8R1WY2TK01N-Alignment.xml")) |> # "Single-file XML2" file from NCBI BLASTn output is expected
  XML_to_DF()
...
```{r}
query_df_80 <- query_df |> filter(bit_score >= 80) # Keep only significant matches
dim(query_df_80)[1]
...
query_df_80_PREDICTED_unique_test <- query_df_80_PREDICTED |>
  filter(!duplicated(query_number)) # Keep the first match for each locus
dim(query_df_80_PREDICTED_unique)[1]

query_df_80_PREDICTED_unique_unique <- query_df_80_PREDICTED_unique |>
